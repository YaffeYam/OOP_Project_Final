...
class GamingStore:
    ...
    def create_account(self, user_id, first_name, last_name, username, password, role="Basic User"):
...
def register():
    if request.method == 'POST':
        ...
        role = data.get('role', 'Basic User')
        
        user_id = len(store.users) + 1
        user = store.create_account(user_id, first_name, last_name, username, password, role)
        if user:
...
def view_library():
    ...
    user = store.get_user_by_id(user_id)
    return jsonify(user.purchase_history)
...
def buy_game():
    ...
    if game:
        if user.balance >= game.price and game.stock > 0:
            ...
            user.add_purchase(game.title)
            db.session.commit()
            return jsonify({'message': 'Game purchased successfully', 'new_balance': user.balance})
        else:
            return jsonify({'message': 'Insufficient balance or game out of stock'}), 400
    else:
...
def send_gift():
    ...
    if receiver and game:
        if sender.balance >= game.price and game.stock > 0:
            ...
            receiver.add_purchase(game.title)
            db.session.commit()
            return jsonify({'message': 'Game gifted successfully'})
        else:
            return jsonify({'message': 'Insufficient balance or game out of stock'}), 400
    else:
        return jsonify({'message': 'Receiver or game not found'}), 404
